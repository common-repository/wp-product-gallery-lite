 ...
 * */
defined( 'WPPG_VERSION' ) or define( 'WPPG_VERSION', '1.1.4' ); //plugin version
...
if ( ! class_exists( 'WPPG_Class' ) ) {
    ...
    class WPPG_Class{
        ...
        function __construct(){
            ...
            add_action('save_post',array($this, 'wppg_individual_product_settings_save' ) );
            ...
            add_action('add_meta_boxes', array($this, 'wppg_individual_product_metabox') ); //added metabx for individual product 
...
//adding Blog metabox
        ...
        //adding metabox for individual custom link
        function wppg_individual_product_metabox(){
            add_meta_box(
                'wppg_individual_product_metabox',                 // Unique ID
                __( 'WPPG Product Settings', WPPG_TD ),      // Box title 'wppg_add_blog', __( 'WP Product Gallery', WPPG_TD )
                array( $this ,'wppg_individual_product_metabox_content' ),  // Content callback, must be of type callable
                'product'                            // Post type
            );
        
        }
        
        function wppg_individual_product_metabox_content(){ 
            global $post;
            $wppg_advance_option = get_post_meta( $post->ID, 'wppg_advance_option', true);
            wp_nonce_field(basename( __FILE__ ), 'wppg_individual_product_nonce');
            ?>
            <div class="wppg-individual-product-settings-wrapper">
                <div class="wppg_individual_product_setting">
                    <label for="wppg-individual-custom-link-one"><?php _e('Individual Custom Link One', WPPG_TD); ?></label>
                    <div class="wppg-tooltip-icon">
                        <span class="dashicons dashicons-info"></span>
                        <span class="wppg-tooltip-info"><?php _e( 'This is used to show the individual custom link for call to action button one', WPPG_TD ); ?></span>
                    </div>
                    <input type="text" name="wppg_advance_option[button_one_custom_link]" id="wppg-individual-custom-link-one" value="<?php echo isset($wppg_advance_option['button_two_custom_link']) ? esc_url($wppg_advance_option['button_one_custom_link']):'' ?>">
                </div>
                <div class="wppg_individual_product_setting">
                    <label for="wppg-individual-custom-link-two"><?php _e('Individual Custom Link Two',WPPG_TD);?></label>
                    <div class="wppg-tooltip-icon">
                        <span class="dashicons dashicons-info"></span>
                        <span class="wppg-tooltip-info"><?php _e( 'This is used to show the individual custom link for call to action button two', WPPG_TD ); ?></span>
                    </div>
                    <input type="text" name="wppg_advance_option[button_two_custom_link]" id="wppg-individual-custom-link-two" value="<?php echo isset($wppg_advance_option['button_two_custom_link']) ? esc_url($wppg_advance_option['button_two_custom_link']):'' ?>">
                </div>
            </div>
...
// save data
        ...
        //save individual custom link metabox
        function wppg_individual_product_settings_save($post_id){
//check individual custom link metabox            
            $is_autosave = wp_is_post_autosave($post_id);
            $is_revision = wp_is_post_revision( $post_id );
            $is_valid_nonce = (isset( $post['wppg_individual_product_nonce']) && wp_verify_nonce($post[ 'wppg_individual_product_nonce' ], basename( __FILE__ ) ) ) ? 'true' : 'false';
            // Exits script depending on save status
            if ( $is_autosave || $is_revision || ! $is_valid_nonce || ! current_user_can( 'edit_posts' ) ) {
                return;
            }

            if ( isset( $_POST[ 'wppg_advance_option' ] ) ) {

                $wppg_array = ( array ) $_POST[ 'wppg_advance_option' ];

                $val = $this -> wppg_sanitize_array( $wppg_array );
// save data
                update_post_meta( $post_id, 'wppg_advance_option', $val );
